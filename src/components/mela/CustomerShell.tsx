      ...
      <div className="bg-slate-900/90 text-white backdrop-blur-md border-b border-white/5 py-1 relative z-50">
        <div className="max-w-[1500px] mx-auto px-2 md:px-4 flex items-center gap-2 md:gap-4 h-14 md:h-16">
          ...
          <div className="flex items-center gap-1">
            ...
            <Link to="/" className="flex items-center gap-0.5 hover:bg-white/10 p-1 md:p-1.5 md:px-3 rounded-xl transition-all group shrink-0">
                <span className="font-display text-2xl md:text-3xl font-black tracking-tighter italic">MELA</span>
          ...
          <div className="relative hidden lg:block">
            ...
            >
              ...
              <div className="flex flex-col">
                <span className="text-[10px] text-slate-400 font-bold uppercase tracking-widest leading-none">Deliver to</span>
                <span className="text-sm font-bold leading-none mt-1 whitespace-nowrap">{district}</span>
              </div>
            ...
            {locOpen && (
              <div className="absolute top-full left-0 mt-3 w-72 bg-white border border-slate-200 shadow-2xl rounded-2xl p-4 text-slate-900 z-[110] animate-in fade-in slide-in-from-top-2 duration-200">
                <div className="px-2 py-2 mb-3">
                   <div className="text-xs font-black text-slate-400 uppercase tracking-[0.15em]">Select Distribution Hub</div>
                </div>
                <div className="grid grid-cols-1 gap-1">
                    {districts.map(d => (
                    <button 
                        key={d} 
                        onClick={() => { setDistrict(d.split(' (')[0]); setLocOpen(false); }}
                    ...
                    >
                        {d}
                        {district === d.split(' (')[0] && <div className="w-2 h-2 rounded-full bg-blue-600 shadow-[0_0_8px_rgba(37,99,235,0.6)]" />}
                    </button>
                    ))}
                </div>
              </div>
          ...
          <div className="flex-1 flex h-11 md:h-12 group overflow-hidden rounded-xl bg-white/10 border border-white/10 focus-within:ring-2 focus-within:ring-blue-500/50 shadow-inner transition-all">
            ...
            <button className="bg-blue-600 hover:bg-blue-700 text-white px-6 transition-all shrink-0 flex items-center justify-center shadow-lg group">
              <Search className="w-5 h-5 md:w-6 md:h-6 group-hover:scale-110 transition-transform" strokeWidth={3} />
            </button>
          </div>
          ...
          <div className="flex items-center gap-1 md:gap-2 shrink-0">
            ...
            <div className="relative group hover:bg-white/10 p-2 px-3 rounded-xl transition-all cursor-pointer flex flex-col justify-center border border-transparent hover:border-white/5">
              <div className="flex flex-col">
                <span className="text-[10px] text-slate-400 font-bold uppercase tracking-widest leading-none">Account</span>
                <span className="text-sm font-bold leading-none mt-1 flex items-center">
                   <span className="hidden sm:inline whitespace-nowrap">{user ? user.name.split(' ')[0] : 'Sign in'}</span>
              ...
              {!user && (
                <div className="absolute top-full right-0 mt-3 w-64 bg-white border border-slate-200 shadow-2xl rounded-2xl py-5 px-6 invisible group-hover:visible z-[110] animate-in fade-in slide-in-from-top-2 duration-200">
                  <Link to="/login" className="block w-full mela-button-primary text-center py-3 mb-4">
                    Sign in securely
                  </Link>
              ...
              {user && (
                 <div className="absolute top-full right-0 mt-3 w-72 bg-slate-900 shadow-2xl rounded-2xl py-4 invisible group-hover:visible z-[110] border border-white/10 overflow-hidden">
                    <div className="px-6 py-2 border-b border-white/5 mb-2">
                       <div className="text-xs font-black text-slate-400 uppercase tracking-widest">Trade Account</div>
                    </div>
                    <Link to="/orders" className="block px-6 py-3 hover:bg-white/5 text-sm font-bold text-slate-200 transition-colors">Your Orders</Link>
                    <Link to="/buyer/account" className="block px-6 py-3 hover:bg-white/5 text-sm font-bold text-slate-200 transition-colors">Trade Account</Link>
                    <div className="h-px bg-white/5 my-2" />
                    <button onClick={() => { logout(); nav({ to: "/" }); }} className="block w-full text-left px-6 py-3 hover:bg-red-500/10 text-sm font-bold text-red-400 transition-colors">Sign Out</button>
                 </div>
              )}
            </div>

            {/* Orders */}
            <Link to="/orders" className="hidden xl:flex flex-col hover:bg-white/10 p-2 px-3 rounded-xl transition-all border border-transparent hover:border-white/5">
              <span className="text-[10px] text-slate-400 font-bold uppercase tracking-widest leading-none">Returns</span>
              <span className="text-sm font-bold leading-none mt-1">& Orders</span>
            </Link>

            {/* Cart */}
            <Link to="/cart" className="flex items-center hover:bg-white/10 p-2 px-3 rounded-xl transition-all relative border border-transparent hover:border-white/5 group">
      ...
      <div className="bg-slate-800/90 text-white h-11 flex items-center overflow-x-auto no-scrollbar scroll-smooth backdrop-blur-md border-b border-white/5 relative z-30">
        <div className="max-w-[1500px] mx-auto px-2 md:px-4 flex items-center gap-3 md:gap-6 w-full text-xs md:text-sm font-bold whitespace-nowrap">
          <Sheet>
            ...
            <SheetContent side="left" className="w-[300px] sm:w-[400px] bg-slate-950 border-r border-white/10 p-0 text-white">
              <SheetHeader className="p-6 bg-slate-900 border-b border-white/5">
                <SheetTitle className="text-white flex items-center gap-2">
                  <User className="w-5 h-5 text-blue-500" />
                  <span className="font-black tracking-tight italic uppercase">Hello, {user ? user.name.split(' ')[0] : 'Sign in'}</span>
                </SheetTitle>
              </SheetHeader>
              <div className="py-6 overflow-y-auto max-h-[calc(100vh-80px)]">
                <div className="px-6 mb-6">
                  <h3 className="text-blue-500 font-black text-[10px] uppercase tracking-[0.2em] mb-4">Trade Sectors</h3>
                ...
                <div className="px-6">
                  <h3 className="text-blue-500 font-black text-[10px] uppercase tracking-[0.2em] mb-4">Account & Settings</h3>
                  <div className="space-y-1 text-sm font-bold">
                    <Link to="/buyer/account" className="block px-4 py-3 rounded-xl hover:bg-white/5">Trade Account</Link>
                    <Link to="/orders" className="block px-4 py-3 rounded-xl hover:bg-white/5">Order Tracking</Link>
                    <Link to="/contact" className="block px-4 py-3 rounded-xl hover:bg-white/5">Enterprise Support</Link>
                    {user && (
                      <button onClick={logout} className="w-full text-left px-4 py-3 rounded-xl hover:bg-red-500/10 text-red-400">Sign Out</button>
                    )}
                  </div>
                </div>
              </div>
          ...
          <NavL to="/catalogue">Catalogue</NavL>
          <NavL to="/catalogue" params={{ filter: 'deals' }}>Trade Deals</NavL>
          <NavL to="/catalogue" params={{ filter: 'new' }}>New arrivals</NavL>
          <NavL to="/contact" className="hidden sm:block">Support</NavL>
          <NavL to="/catalogue" className="hidden lg:block">Logistics</NavL>
...
function NavL({ to, children, className, params, search }: { to: string; children: ReactNode; className?: string; params?: any; search?: any }) {
  return (
    <Link 
      to={to} 
      params={params}
  ...
  return (
    <footer className="bg-slate-950 text-white mt-12 font-sans overflow-x-hidden relative">
      ...
      <div className="max-w-[1200px] mx-auto px-6 py-20 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-12 text-center sm:text-left relative z-10">
        <div>
          <h4 className="font-black text-xs mb-8 uppercase tracking-[0.2em] text-blue-500">Corporate Hub</h4>
          <ul className="text-sm text-slate-400 space-y-4">
            <li><Link to="/about" className="hover:text-white transition-colors">About MELA Hub</Link></li>
            <li><Link to="/" className="hover:text-white transition-colors">Trade Solutions</Link></li>
            <li><Link to="/" className="hover:text-white transition-colors">Logistics Network</Link></li>
            <li><Link to="/" className="hover:text-white transition-colors">Digital Ecosystem</Link></li>
          </ul>
        </div>
        <div>
          <h4 className="font-black text-xs mb-8 uppercase tracking-[0.2em] text-blue-500">Partnerships</h4>
          <ul className="text-sm text-slate-400 space-y-4">
            <li><Link to="/admin" className="hover:text-white transition-colors">Admin Portal</Link></li>
            <li><Link to="/vendor" className="hover:text-white transition-colors">Vendor / Warehouse</Link></li>
            <li><Link to="/" className="hover:text-white transition-colors">Verified Logistics</Link></li>
          </ul>
        </div>
        <div>
          <h4 className="font-black text-xs mb-8 uppercase tracking-[0.2em] text-blue-500">Financials</h4>
          <ul className="text-sm text-slate-400 space-y-4">
            <li><Link to="/" className="hover:text-white transition-colors">MELA Trade Credit</Link></li>
            <li><Link to="/" className="hover:text-white transition-colors">Direct Settlement</Link></li>
            <li><Link to="/" className="hover:text-white transition-colors">Invoice Lifecycle</Link></li>
            <li><Link to="/" className="hover:text-white transition-colors">Currency Exchange</Link></li>
          </ul>
        </div>
        <div>
          <h4 className="font-black text-xs mb-8 uppercase tracking-[0.2em] text-blue-500">Resource Center</h4>
          <ul className="text-sm text-slate-400 space-y-4">
            <li><Link to="/buyer/account" className="hover:text-white transition-colors">Your Account</Link></li>
            <li><Link to="/orders" className="hover:text-white transition-colors">Supply Chain Tracking</Link></li>
            <li><Link to="/" className="hover:text-white transition-colors">Hub Operations</Link></li>
            <li><Link to="/contact" className="hover:text-white transition-colors">Enterprise Support</Link></li>
          </ul>
        </div>
      </div>
      ...
      <div className="border-t border-white/5 py-16 relative z-10">
        <div className="flex flex-col items-center gap-8">
          <div className="flex items-center gap-0.5 group cursor-pointer">
             ...
             <span className="text-blue-500 font-black text-3xl mt-2 group-hover:translate-x-1 transition-transform">.uk</span>
          </div>
          <div className="flex flex-wrap justify-center gap-3 text-[10px] text-slate-500 font-black uppercase tracking-[0.15em]">
            <span className="border border-white/10 px-6 py-3 rounded-xl hover:bg-white/5 hover:text-white transition-all cursor-pointer backdrop-blur-md">Global - English</span>
            <span className="border border-white/10 px-6 py-3 rounded-xl hover:bg-white/5 hover:text-white transition-all cursor-pointer backdrop-blur-md">GBP - Sterling</span>
            <span className="border border-white/10 px-6 py-3 rounded-xl hover:bg-white/5 hover:text-white transition-all cursor-pointer backdrop-blur-md">HQ - United Kingdom</span>
          </div>
        </div>
      </div>
      ...
      <div className="bg-slate-950 py-12 px-6 text-center text-[10px] text-slate-600 border-t border-white/5 relative z-10">
        <div className="flex flex-wrap justify-center gap-x-8 gap-y-3 mb-6 font-bold uppercase tracking-widest">
          <Link to="/" className="hover:text-blue-400 transition-colors">Governance</Link>
          <Link to="/" className="hover:text-blue-400 transition-colors">Privacy Framework</Link>
          <Link to="/" className="hover:text-blue-400 transition-colors">Trade Cookies</Link>
          <Link to="/" className="hover:text-blue-400 transition-colors">Accessibility</Link>
        </div>
        <div className="font-black tracking-[0.05em] uppercase text-slate-700">© 2024-2026, MELA GLOBAL TRADE NETWORK. ALL RIGHTS RESERVED.</div>
      </div>
    </footer>
  );
}
