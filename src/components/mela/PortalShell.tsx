...
import { Menu, X, LogOut, ChevronRight, LayoutDashboard, Database, ClipboardList, Users, Shield, ArrowRight, Settings, Building2 } from "lucide-react";
...
export function PortalShell({ portal, role, accent, nav, right }: Props) {
  ...
  const Sidebar = (
    <div className="h-full flex flex-col bg-slate-950 text-white">
      {/* Sidebar Header */}
      <Link to="/" className="px-6 py-8 flex items-center gap-3 border-b border-white/5">
        <div className="w-10 h-10 rounded-xl bg-blue-600 text-white grid place-items-center font-black text-2xl shadow-lg shadow-blue-500/20 italic tracking-tighter">M</div>
        <div>
          <div className="font-display text-2xl font-black tracking-tighter leading-none italic">MELA</div>
          <div className="text-[10px] uppercase tracking-[0.2em] text-slate-500 font-bold mt-1.5 flex items-center gap-2">
            <span className={`w-2 h-2 rounded-full ${accent} animate-pulse shadow-[0_0_8px_rgba(59,130,246,0.5)]`} /> {portal} Hub
          </div>
        </div>
      </Link>
      ...
      <nav className="flex-1 px-4 py-8 space-y-1.5 overflow-y-auto">
        <div className="text-[10px] font-black uppercase tracking-widest text-slate-600 px-3 mb-6">Portal Matrix</div>
        {nav.map((n) => {
      ...
      <div className="p-6 bg-slate-900/50 border-t border-white/5 space-y-4">
        <div className="flex items-center gap-3">
           ...
           <div className="min-w-0">
              ...
              <div className="text-[10px] text-slate-500 font-black uppercase tracking-widest truncate">{displayOrg}</div>
           </div>
        </div>
        <button
          onClick={() => { logout(); nav2({ to: "/" }); }}
  ...
  return (
    <div className="min-h-screen flex bg-slate-50 font-sans">
      ...
      <main className="flex-1 min-w-0 flex flex-col">
        ...
        <header className="h-16 border-b border-slate-200 px-4 sm:px-6 lg:px-10 flex items-center justify-between bg-white/80 backdrop-blur-md sticky top-0 z-30 gap-4">
          <div className="flex items-center gap-4 min-w-0">
            ...
            <div className="flex items-center gap-2 overflow-hidden">
               <span className="text-[10px] font-black uppercase tracking-widest text-slate-400 whitespace-nowrap hidden sm:inline">Central Hub</span>
               <ChevronRight className="w-3 h-3 text-slate-300 shrink-0 hidden sm:inline" />
               <span className="text-xs font-bold text-slate-900 truncate uppercase italic tracking-tight">{portal} Portal · London District</span>
            </div>
          </div>
        ...
        {/* Portal Content */}
        <div className="p-4 sm:p-8 lg:p-12 max-w-[1200px] w-full mx-auto">
          <Outlet />
        </div>
      </main>
    </div>
  ...
  return (
    <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10 md:mb-12 border-b border-slate-200 pb-10">
      <div className="min-w-0 flex-1">
        {eyebrow && <div className="text-[10px] font-black uppercase tracking-[0.3em] text-blue-600 mb-4">{eyebrow}</div>}
        <h1 className="text-3xl sm:text-4xl md:text-6xl font-black text-slate-900 tracking-tighter leading-none italic uppercase break-words">{title}</h1>
        {sub && <p className="text-slate-500 mt-4 max-w-2xl text-xs sm:text-sm md:text-base font-medium leading-relaxed">{sub}</p>}
      </div>
...
export function StatCard({ label, value, hint }: { label: string; value: string | number; hint?: string }) {
  return (
    <div className="bg-white rounded-2xl border border-slate-200 p-8 shadow-sm hover:shadow-xl hover:shadow-blue-500/5 transition-all group">
      <div className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 mb-3 group-hover:text-blue-600 transition-colors">{label}</div>
      <div className="text-4xl md:text-5xl font-black text-slate-900 tracking-tighter">{value}</div>
      {hint && <div className="text-[10px] text-emerald-600 font-black uppercase tracking-widest mt-4 flex items-center gap-2">
         <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse shadow-[0_0_8px_rgba(16,185,129,0.5)]" /> {hint}
      </div>}
    </div>
  );
}
...
export function Pill({ children, tone = "default" }: { children: ReactNode; tone?: "default" | "good" | "warn" | "bad" | "info" }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full border text-[10px] font-black uppercase tracking-tighter ${tones[tone]}`}>
       {children}
    </span>
  );
}
