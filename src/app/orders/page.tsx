    ...
    <div className="bg-white min-h-screen font-sans pb-20">
      <div className="max-w-[1000px] mx-auto px-4 py-8 md:py-12">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-8 border-b border-gray-100 pb-6">
          <div>
            <h1 className="text-3xl font-bold text-amazon-dark tracking-tight">
              Your Trade Orders
            </h1>
            <p className="text-gray-500 text-sm mt-1 font-medium uppercase tracking-widest">
              Tracking shipments from MELA-01 Park Royal
            </p>
          </div>
          <div className="flex gap-4 text-xs font-bold uppercase tracking-widest text-gray-400">
            <button className="text-amazon-orange border-b-2 border-amazon-orange pb-1">
              Active
            </button>
        ...
        <div className="space-y-6">
          {orders.map((o) => (
            <div
              key={o.id}
              className="border border-gray-200 rounded-lg shadow-sm overflow-hidden group hover:border-gray-300 transition-colors"
            >
                ...
                <div className="flex-1 space-y-4 w-full">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                    <div className="flex items-center gap-3">
                      <span
                        className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-tighter border ${getStatusColor(o.status)}`}
                      >
                        {o.status}
                      </span>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex justify-between text-[9px] font-bold text-gray-400 uppercase tracking-widest px-1">
                      <div className={o.status === "Confirmed" ? "text-amazon-orange" : ""}>
                        Confirmed
                      </div>
                      <div className={o.status === "Picking" ? "text-amazon-orange" : ""}>
                        Picking
                      </div>
                      <div className={o.status === "Dispatched" ? "text-amazon-orange" : ""}>
                        In Transit
                      </div>
                      <div className={o.status === "Delivered" ? "text-amazon-orange" : ""}>
                        Delivered
                      </div>
                    </div>
                  </div>
              ...
              <div className="px-6 py-3 bg-gray-50/50 border-t border-gray-100 flex items-center gap-2">
                ...
                <span className="text-[10px] text-gray-500 font-medium">
                  Originating Facility: <span className="font-bold">{o.warehouse}</span>
                </span>
              </div>
            </div>
          ))}
...
function SignInPrompt({ page }: { page: string }) {
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50 p-6 text-center font-sans">
      <div className="max-w-[400px] bg-white border border-gray-200 rounded-lg p-10 shadow-sm">
        <h1 className="text-3xl font-bold text-amazon-dark mb-4 tracking-tight">
          Sign-In Required
        </h1>
        <p className="text-gray-600 text-sm leading-relaxed mb-8">
          To access your <span className="font-bold">{page}</span> and manage your trade account,
          please authenticate using your registered business credentials.
        </p>
        <Link
          href="/login"
          className="block w-full bg-amazon-orange hover:bg-amazon-orange-hover text-amazon-dark py-3 rounded-md text-sm font-bold shadow-md transition-all active:scale-95"
        >
