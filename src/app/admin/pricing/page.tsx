...
import { Package, BarChart3, Plus, History, Download } from "lucide-react";
...
export default function Page() {
  const [priceHistory, setPriceHistory] = useState(
    products.map((p) => ({
      ...p,
      history: [
        { date: "2026-04-01", price: 24.5, trend: "up" as const },
        { date: "2026-04-15", price: 23.9, trend: "down" as const },
        { date: "2026-05-01", price: 25.2, trend: "up" as const },
      ],
    })),
  );
    ...
    setPriceHistory((prev) =>
      prev.map((p) => {
        if (p.id === selectedProduct.id) {
          return { ...p, history: [...p.history, { date: newDate, price, trend }] };
        }
        return p;
      }),
    );
  ...
  const handleDownloadReport = () => {
    ...
    const rows = [
      ...
      ...priceHistory.map((p) => [
        ...
        p.name,
        p.unit,
        p.history[p.history.length - 1].price.toString(),
        p.history[p.history.length - 1].date,
      ]),
    ];
    ...
    link.setAttribute(
      "download",
      `mela_pricing_report_${new Date().toISOString().split("T")[0]}.csv`,
    );
          ...
          <tbody className="divide-y divide-slate-100">
            {priceHistory.map((p) => (
              <tr key={p.id} className="hover:bg-slate-50/50 transition-colors group">
                ...
                <td className="px-6 py-5 text-sm font-medium text-slate-600 italic">{p.unit}</td>
                <td className="px-6 py-5">
                  <div className="flex items-baseline gap-1">
                    <span className="text-[10px] font-black text-slate-400 uppercase">£</span>
                    <span className="text-lg font-black text-slate-900">
                      {p.history[p.history.length - 1].price.toFixed(2)}
                    </span>
                  </div>
                </td>
                <td className="px-6 py-5">
                  <div className="flex gap-4">
                    {p.history.slice(-4).map((h, i) => (
                      <div key={i} className="flex flex-col items-center gap-1 group/item relative">
                        <div
                          className={`w-2 h-2 rounded-full ${h.trend === "up" ? "bg-emerald-500" : "bg-amber-500"}`}
                        />
      ...
      <div className="grid md:grid-cols-3 gap-6">
        <div className="bg-slate-950 p-8 rounded-2xl text-white shadow-xl">
          ...
          <p className="text-xs text-slate-400 mt-4 font-medium leading-relaxed">
            Anticipated variance across F&B sectors due to logistics escalation in London NW10.
          </p>
        </div>
      </div>
