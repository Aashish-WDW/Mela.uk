...
import { MapPin, Star, Plus, Search, Filter, Mail } from "lucide-react";
    ...
    <div className="space-y-8">
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-4 flex flex-col md:flex-row gap-4 items-center justify-between shadow-sm">
        <div className="relative w-full md:w-96">
          ...
          <input
            placeholder="Search vendor network..."
      ...
      <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-6">
        {vendors.map((v) => (
          <div
            key={v.id}
            className="bg-white border border-gray-200 rounded-lg shadow-sm hover:shadow-md transition-shadow flex flex-col overflow-hidden group"
          >
            <div className="p-6 flex-1">
              ...
              <p className="text-sm text-gray-600 leading-relaxed line-clamp-2 italic mb-6">
                "{v.blurb}"
              </p>
              ...
              <div className="grid grid-cols-2 gap-4 border-t border-gray-50 pt-4 mt-auto">
                ...
                <div className="text-center border-l border-gray-50">
                  ...
                  <div className="text-[10px] font-black text-green-700 uppercase tracking-tighter">
                    Active Trade
                  </div>
                </div>
              </div>
            ...
            <div className="bg-gray-50 px-6 py-3 border-t border-gray-100 flex items-center justify-between gap-3">
              <button
                onClick={() => flash(`Viewing ${v.name} Inventory`)}
              ...
              <button
                ...
                className="text-[10px] font-black text-amazon-dark hover:text-amazon-orange uppercase tracking-widest flex items-center gap-1.5 transition-colors"
              >
      ...
      <Modal
        ...
        title={`Vendor Enquiry: ${contact?.name ?? ""}`}
        description="Communications are monitored for quality assurance."
        size="lg"
      >
        <form
          onSubmit={(e) => {
            e.preventDefault();
            flash(`Message dispatched to ${contact?.name}`);
            setContact(null);
          }}
          className="space-y-6"
        >
          <Field label="Transmission Subject">
            <input
              required
              className="amazon-input"
              value={subject}
              onChange={(e) => setSubject(e.target.value)}
              placeholder="e.g. Inventory Audit NW10"
            />
          </Field>
          <Field label="Communication Body">
            <textarea
              ...
              placeholder="Enter detailed message for vendor operations lead..."
            />
          </Field>
      ...
      <Modal
        ...
        description="Initiate the verification process for a new supply partner."
        size="lg"
      >
        <form
          onSubmit={(e) => {
            e.preventDefault();
            flash(`Invitation dispatched to ${vEmail}`);
            setInvite(false);
            setVName("");
            setVEmail("");
          }}
        ...
        >
          ...
          <Field label="Operational Hub City">
            <input
              required
              className="amazon-input"
              value={vCity}
