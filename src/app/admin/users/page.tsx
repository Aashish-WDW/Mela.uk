...
import { Modal } from "@/components/mela/Modal";
...
export default function Page() {
  ...
  const uniqueDates = Array.from(new Set(salesHistory.map((s) => s.date)))
    .sort()
    .reverse();
  ...
  const filteredSales = selectedDate ? salesHistory.filter((s) => s.date === selectedDate) : [];
  ...
  const handleDownloadAudit = () => {
    ...
    link.setAttribute(
      "download",
      `mela_audit_${selectedUser.id}_${new Date().toISOString().split("T")[0]}.csv`,
    );
    ...
    <div className="space-y-8">
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
        {buyers.map((u) => (
          <div
            key={u.id}
            className="bg-white border border-slate-200 rounded-2xl shadow-sm hover:shadow-xl transition-all group cursor-pointer overflow-hidden"
      ...
      >
        <div className="space-y-8">
          ...
          <div>
            <div className="border border-slate-200 rounded-xl overflow-hidden shadow-inner bg-slate-50 overflow-x-auto">
                ...
                <tbody className="divide-y divide-slate-100 bg-white">
                  {filteredSales.length > 0 ? (
                    ...
                    <tr>
                      <td
                        colSpan={3}
                        className="px-4 py-8 text-center text-xs font-bold text-slate-400 italic"
                      >
                        No transactional data for this date.
                      </td>
                    </tr>
                  )}
                </tbody>
          ...
          <div className="flex justify-between items-center pt-6 border-t border-slate-100">
            ...
            <button onClick={() => setSelectedUser(null)} className="mela-button-primary px-10">
              Finalize Audit
            </button>
          </div>
        </div>
