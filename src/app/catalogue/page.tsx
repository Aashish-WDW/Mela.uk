    ...
    <div className="bg-white min-h-screen font-sans">
          ...
          <div className="flex items-center justify-between">
            <div>
              <span className="text-gray-600">
                1-{filtered.length} of over {products.length} results for{" "}
              </span>
          ...
          <div className="flex items-center justify-between md:justify-end gap-4 border-t md:border-t-0 pt-2 md:pt-0">
            <div className="flex items-center border border-gray-300 rounded-md bg-gray-100 p-0.5">
              <button
                ...
                className={`p-1 px-2 rounded-sm transition-all ${view === "grid" ? "bg-white shadow-sm" : "hover:bg-gray-200 opacity-60"}`}
              >
                <LayoutGrid className="w-4 h-4" />
              </button>
              <button
                onClick={() => setView("list")}
                className={`p-1 px-2 rounded-sm transition-all ${view === "list" ? "bg-white shadow-sm" : "hover:bg-gray-200 opacity-60"}`}
              >
      ...
      <div className="max-w-[1500px] mx-auto grid grid-cols-1 md:grid-cols-12 gap-8 px-4 py-6">
        <aside className="md:col-span-3 lg:col-span-2 space-y-8 hidden md:block">
          <section>
            <h3 className="font-bold text-[13px] mb-3 uppercase tracking-wider text-gray-900">
              Department
            </h3>
            ...
            <div className="space-y-2">
              {[4, 3, 2, 1].map((stars) => (
                <div key={stars} className="flex items-center gap-1.5 group cursor-pointer">
                  ...
                  <span className="text-xs text-gray-600 group-hover:text-orange-700 font-medium">
                    & Up
                  </span>
                </div>
              ))}
          ...
          >
            {filtered.map((p) => {
              ...
              return (
                ...
                >
                  <div
                    className={`${view === "grid" ? "aspect-square mb-4" : "w-32 h-32 md:w-56 md:h-56 shrink-0"} relative bg-gray-50 rounded-md overflow-hidden flex items-center justify-center border border-gray-100 group-hover:shadow-md transition-all`}
                  >
                    <img
                      src={p.image}
                      alt={p.name}
                    ...
                    <div className="absolute top-2 left-2 flex flex-col gap-1">
                      {p.tags.slice(0, 1).map((t) => (
                        <span
                          key={t}
                          className="bg-white/90 backdrop-blur-sm text-[8px] font-bold border border-gray-200 px-1.5 py-0.5 rounded-sm shadow-sm uppercase"
                        >
                          {t}
                        </span>
                      ))}
                  ...
                  <div className="flex-1 flex flex-col">
                    ...
                    <div className="text-[11px] text-gray-500 mb-2 font-medium">
                      Sold by: <span className="text-blue-600 hover:underline">{v?.name}</span>
                    </div>
                    ...
                    <div className="flex items-center gap-1.5 mb-2.5">
                      ...
                      <span className="text-[11px] text-blue-600 hover:underline">
                        {(p.stock % 50) + 20} verified reviews
                      </span>
                    </div>
                    ...
                    <div className="mt-auto space-y-1.5">
                      ...
                      <div className="flex items-center gap-2 text-[11px] text-gray-700 bg-gray-50 w-fit px-2 py-0.5 rounded-sm border border-gray-100">
                        <span className="font-bold text-orange-800 uppercase tracking-tighter">
                          Prime
                        </span>
                      ...
                      <div className="text-[11px] text-gray-600">
                        FREE Trade delivery for verified businesses
                      </div>
                    </div>
                  </div>
                </Link>
          ...
          {filtered.length === 0 && (
            <div className="text-center py-20 bg-gray-50 rounded-lg border border-dashed border-gray-300 mx-4 md:mx-0">
              ...
              <button
                ...
                className="bg-white border border-gray-300 px-6 py-2 rounded-md text-sm font-bold shadow-sm hover:bg-gray-50 transition-colors"
              >
                Clear all filters
              </button>
            </div>
