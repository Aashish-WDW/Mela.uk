        ...
        <div className="max-w-[1200px] mx-auto flex flex-col lg:flex-row gap-6">
          <div className="flex-1 bg-white p-6 md:p-10 shadow-sm rounded-sm">
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <Link
                href="/catalogue"
                className="bg-amazon-orange hover:bg-amazon-orange-hover text-amazon-dark px-8 py-2.5 rounded-md text-sm font-bold shadow-sm transition-colors text-center"
              >
                Continue shopping
              </Link>
              {!user && (
                <Link
                  href="/login"
                  className="border border-gray-300 hover:bg-gray-50 px-8 py-2.5 rounded-md text-sm font-bold shadow-sm transition-colors text-center bg-white"
                >
          ...
          <div className="w-full lg:w-80 space-y-4">
            <div className="bg-white p-5 shadow-sm rounded-sm border-t-2 border-amazon-orange">
              <h3 className="font-bold text-sm mb-4 uppercase tracking-tighter text-gray-800">
                Trade Intelligence
              </h3>
              <p className="text-xs text-gray-500 leading-relaxed italic">
                "Markets move fast. We recommend reviewing your inventory levels every 48 hours for
                optimal fulfilment."
              </p>
            </div>
          </div>
        ...
        <div className="lg:col-span-3 bg-white p-4 md:p-8 shadow-sm rounded-sm">
          ...
          <div className="space-y-8">
            {items.map((it) => {
              ...
              return (
                <div
                  key={it.id}
                  className="flex flex-col md:flex-row gap-4 md:gap-6 border-b border-gray-100 pb-8 last:border-0"
                >
                  <Link
                    href={`/product/${p.id}`}
                    className="w-full md:w-48 aspect-square shrink-0 bg-gray-50 rounded-md overflow-hidden border border-gray-100 flex items-center justify-center group relative"
                  >
                    <img
                      src={p.image}
                      alt={p.name}
                    ...
                    />
                    <div className="absolute top-2 left-2 bg-white/90 backdrop-blur px-2 py-0.5 rounded shadow-sm text-[10px] font-bold text-orange-800 uppercase tracking-tighter">
                      Prime
                    </div>
                  </Link>
                  ...
                  <div className="flex-1 min-w-0 flex flex-col">
                    <div className="flex flex-col md:flex-row justify-between items-start gap-4">
                      <div className="flex-1">
                        <Link
                          href={`/product/${p.id}`}
                          className="text-[17px] md:text-xl font-medium text-gray-900 hover:text-blue-600 hover:underline line-clamp-2 leading-tight"
                        >
                          {p.name}
                        </Link>
                        ...
                        <div className="text-xs text-gray-600 mt-2">
                          Sold by:{" "}
                          <Link href="/vendor" className="text-blue-600 hover:underline font-bold">
                            {v?.name}
                          </Link>
                        </div>
                    ...
                    <div className="flex items-center gap-2 md:gap-5 mt-6 flex-wrap">
                      <div className="flex items-center bg-gray-50 border border-gray-300 rounded-lg shadow-sm h-9">
                        <button
                          onClick={() => update(it.id, Math.max(p.moq, it.qty - 1))}
                          className="px-3.5 h-full hover:bg-gray-200 text-xl font-medium transition-colors border-r border-gray-300"
                        >
                          −
                        </button>
                        <span className="min-w-[40px] px-2 text-center text-sm font-bold h-full flex items-center justify-center bg-white">
                          {it.qty}
                        </span>
                        <button
                          onClick={() => update(it.id, it.qty + 1)}
                          className="px-3.5 h-full hover:bg-gray-200 text-xl font-medium transition-colors border-l border-gray-300"
                        >
                          +
                        </button>
                      </div>
                      ...
                      <div className="flex items-center gap-3 text-xs font-medium">
                        <button
                          ...
                          className="flex items-center gap-1.5 text-blue-600 hover:underline border-l md:border-l-0 pl-3 md:pl-0 border-gray-300"
                        >
          ...
          <div className="flex justify-between items-center mt-10 pt-6 border-t border-gray-100">
            ...
            <div className="text-lg md:text-xl font-medium">
              ...
              <span className="font-bold ml-1 text-gray-900">TBD</span>
            </div>
          </div>
        </div>
        ...
        <aside className="space-y-5">
          <div className="bg-white p-6 shadow-sm rounded-sm border border-gray-100">
            <div className="flex items-start gap-3 mb-5 p-3 bg-green-50/50 rounded-md border border-green-100">
              ...
              <div>
                <div className="text-[11px] text-green-800 font-bold uppercase tracking-tight">
                  Trade Shipping Guarantee
                </div>
                <div className="text-[10px] text-gray-600 mt-0.5 leading-tight">
                  Your order qualifies for{" "}
                  <span className="font-bold">Next-Day Hub Fulfilment</span>.
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3 mb-8">
              ...
              <label
                htmlFor="sample"
                className="text-[11px] text-gray-700 font-medium cursor-pointer"
              >
                This enquiry includes a commercial sample request
              </label>
            </div>
            ...
            {user ? (
              <Link
                href="/catalogue"
                className="block w-full bg-amazon-orange hover:bg-amazon-orange-hover text-amazon-dark text-center py-2.5 rounded-full text-sm font-bold shadow-md transition-all transform active:scale-95 mb-4"
              >
            ...
            <div className="mt-6 bg-gray-50/80 p-4 rounded-lg border border-gray-100">
              ...
              <p className="text-[10px] text-gray-500 leading-relaxed font-medium">
                Enquiry pricing is dynamic and reflects current warehouse stock. A Mela account
                manager will contact you within 2 business hours to finalize terms.
              </p>
            </div>
          </div>

          <div className="bg-white p-6 shadow-sm rounded-sm border border-gray-100 hidden lg:block">
            <h3 className="font-bold text-xs mb-5 uppercase tracking-wider text-gray-900 border-b border-gray-50 pb-2">
              Business Favorites
            </h3>
            <div className="space-y-5">
              {products.slice(10, 13).map((p) => (
                <Link key={p.id} href={`/product/${p.id}`} className="flex gap-4 group">
                  ...
                  <div className="flex flex-col justify-center">
                    <div className="text-[11px] font-bold text-blue-600 group-hover:text-orange-700 group-hover:underline line-clamp-2 leading-tight">
                      {p.name}
                    </div>
                    ...
                    <div className="text-[10px] text-gray-500 mt-1.5 font-bold uppercase tracking-tighter">
                      MOQ: {p.moq} {p.unit.split(" ")[0]}
                    </div>
                  </div>
                </Link>
              ))}
            ...
            <Link
              href="/catalogue"
              className="text-[11px] font-bold text-blue-600 hover:underline mt-6 block text-center py-2 border-t border-gray-50"
            >
