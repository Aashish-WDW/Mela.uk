...
import { Boxes, Package, RefreshCw, AlertTriangle, ArrowRight, Search, Filter, History, TrendingDown, ClipboardList, Tag, Calendar } from "lucide-react";
...
function Inventory() {
  const [inventory, setInventory] = useState(
    products.filter((p) => p.vendor === "v-borough" || p.vendor === "v-shoreditch").map(p => ({
      ...p,
      threshold: 100,
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
         <div className="bg-white border border-slate-200 p-6 rounded-2xl shadow-sm">
            <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Total SKUs</div>
         ...
         <div className="bg-white border border-slate-200 p-6 rounded-2xl shadow-sm">
            <div className="text-[10px] font-black text-blue-500 uppercase tracking-widest mb-2">Active Batches</div>
            <div className="text-3xl font-black text-slate-900">{inventory.length * 2}</div>
         </div>
         <div className="bg-white border border-slate-200 p-6 rounded-2xl shadow-sm">
            <div className="text-[10px] font-black text-emerald-500 uppercase tracking-widest mb-2">Hub Space Used</div>
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-4 flex flex-col md:flex-row gap-4 items-center justify-between shadow-sm">
         <div className="relative w-full md:w-96">
            ...
            <input placeholder="Filter by SKU, Batch, or Product Name..." className="w-full pl-10 pr-4 py-2 bg-slate-50 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 outline-none transition-all shadow-inner" />
         </div>
         ...
         <table className="w-full text-left border-collapse min-w-[900px]">
            <thead>
               <tr className="bg-slate-50 border-b border-slate-200">
                  <th className="px-6 py-4 text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">Inventory Item</th>
                  <th className="px-6 py-4 text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">Batch / Expiry</th>
                  <th className="px-6 py-4 text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">On Hand</th>
                  ...
                  return (
                     <tr key={p.id} className="hover:bg-slate-50/50 transition-colors group">
                        ...
                        <td className="px-6 py-5">
                           ...
                           <div className="text-[9px] font-bold text-slate-400 uppercase tracking-tighter">Units in Hub</div>
                        </td>
                        <td className="px-6 py-5">
                           {isLow ? (
      ...
      <Modal open={!!editing} onClose={() => setEditing(null)} title="Inventory Adjustment" description={`Log a physical stock change for ${editing?.sku}`} size="md">
         <form onSubmit={handleUpdate} className="space-y-6">
            <div className="p-4 bg-slate-50 rounded-xl border border-slate-100 flex items-center gap-4 mb-6">
               ...
               <div>
                  ...
                  <div className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">Available: {editing?.stock} Units</div>
               </div>
            </div>

            <Field label="Quantity Adjustment (± Units)">
               <input required type="number" className="mela-input" value={delta} onChange={(e) => setDelta(Number(e.target.value))} placeholder="e.g. +50 or -12" />
            </Field>

            <Field label="Operational Reason">
               <select value={reason} onChange={(e) => setReason(e.target.value)} className="mela-input">
            ...
            <div className="p-4 bg-amber-50 border border-amber-100 rounded-xl flex gap-3">
               ...
               <p className="text-[11px] text-amber-800 leading-relaxed font-medium">
                  This adjustment will be recorded in the hub ledger. Physical verification of Batch <span className="font-bold">{editing?.batch}</span> is required.
               </p>
            </div>

            <div className="flex justify-end gap-3 pt-4 border-t border-slate-100">
               <button type="button" onClick={() => setEditing(null)} className="px-6 py-2.5 rounded-xl border border-slate-300 text-sm font-bold text-slate-600 hover:bg-slate-50">Cancel</button>
      ...
      <Modal open={showHistory} onClose={() => setShowHistory(false)} title="Operational Activity Log" description="Recent inventory movements and manual adjustments." size="lg">
         <div className="space-y-4">
            {[1, 2, 3].map(i => (
               <div key={i} className="flex gap-4 p-4 hover:bg-slate-50 transition-colors rounded-xl border border-transparent hover:border-slate-100">
                  ...
                  <div className="flex-1">
                     ...
                     <p className="text-xs text-slate-500 mt-1">Updated <span className="font-bold text-slate-700">BP-FLOUR-01</span> by <span className="text-emerald-600 font-bold">+50 units</span> (Restock).</p>
                     <div className="flex items-center gap-2 mt-2">
                        <span className="text-[9px] font-black uppercase tracking-widest text-slate-400 bg-slate-100 px-2 py-0.5 rounded">Op: PRIYA.S</span>
                     </div>
                  </div>
               </div>
            ))}
