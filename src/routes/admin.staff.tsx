...
import { UserPlus, Search, Filter, Mail, Briefcase, Building, MoreVertical, Shield } from "lucide-react";
...
function Staff() {
  ...
  const reset = () => { setName(""); setEmail(""); setRole("Trade Desk"); setDept("Sales"); setNote(""); };
  ...
  const submit = (e: React.FormEvent) => {
    ...
    const newStaff: Staff = {
      id: `s${Date.now()}`,
      name, email, role, dept, status: "Invited",
  ...
  return (
    <div className="space-y-8">
      <PageHeader
        ...
        action={
          <button onClick={() => setOpen(true)} className="bg-amazon-orange hover:bg-amazon-orange-hover text-amazon-dark px-6 py-2.5 rounded-md text-sm font-bold shadow-md transition-all active:scale-95 flex items-center gap-2">
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-4 flex flex-col md:flex-row gap-4 items-center justify-between shadow-sm">
         <div className="relative w-full md:w-96">
            ...
            <input placeholder="Filter by name, role or department..." className="w-full pl-10 pr-4 py-2 bg-gray-50 border border-gray-300 rounded-md text-sm focus:ring-1 focus:ring-amazon-orange outline-none transition-all shadow-inner" />
         </div>
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
        {staff.map((s) => (
          <div key={s.id} className="bg-white border border-gray-200 rounded-lg shadow-sm hover:shadow-md transition-all group overflow-hidden flex flex-col">
            <div className="p-6 flex-1">
              <div className="flex items-start justify-between gap-4 mb-6">
                <div className="flex items-center gap-4">
                   ...
                   <div className="min-w-0">
                      <h3 className="font-bold text-lg text-amazon-dark truncate group-hover:text-blue-600 transition-colors">{s.name}</h3>
              ...
              <div className="mt-6 flex items-center justify-between">
                <div className="flex items-center gap-1.5">
                   <div className={`w-2 h-2 rounded-full ${s.status === 'Active' ? 'bg-green-500' : s.status === 'Away' ? 'bg-orange-500' : 'bg-blue-500'} animate-pulse`} />
                   <span className="text-[10px] font-black uppercase tracking-widest text-gray-400">{s.status}</span>
                </div>
                <Pill tone={s.status === "Active" ? "good" : s.status === "Invited" ? "info" : "default"}>{s.status}</Pill>
              </div>
            </div>
      ...
      <Modal open={open} onClose={() => setOpen(false)} title="Teammate Provisioning" description="New staff will receive an encrypted MELA network invitation." size="lg">
        <form onSubmit={submit} className="grid md:grid-cols-2 gap-6">
          <Field label="Full Legal Name"><input required className="amazon-input" value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. Sarah Jenkins" /></Field>
          <Field label="Work Identity (Email)"><input required type="email" className="amazon-input" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="name@mela.london" /></Field>
          ...
          <div className="md:col-span-2">
            <Field label="Security Onboarding Note">
              <textarea rows={3} className="amazon-input" value={note} onChange={(e) => setNote(e.target.value)} placeholder="Briefly describe initial access requirements or welcome message..." />
            </Field>
          </div>
          <div className="md:col-span-2 flex justify-end gap-3 pt-4 border-t border-gray-50">
            <button type="button" onClick={() => setOpen(false)} className="px-6 py-2.5 rounded-md border border-gray-300 text-sm font-bold text-gray-600 hover:bg-gray-50 transition-colors">Abort</button>
            <button type="submit" className="amazon-button-primary px-8">Dispatch Invitation</button>
          </div>
        </form>
      </Modal>
