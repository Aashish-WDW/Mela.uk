    ...
    <div className="space-y-10">
      ...
      <div className="grid lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
          <div className="bg-slate-50 border-b border-slate-200 px-6 py-4 flex items-center justify-between">
             <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] font-black uppercase tracking-widest text-slate-900">Demand Analytics</span>
             </div>
             <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">14 Day Traffic Index</span>
          </div>
            ...
            <div className="flex items-end gap-1.5 h-64">
              {trend.map((v, i) => (
                <div 
                  ...
                  style={{ height: `${v * 0.9}%`, background: i === trend.length - 1 ? '#2563eb' : '#f1f5f9' }} 
                >
        ...
        <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden flex flex-col">
          <div className="bg-slate-50 border-b border-slate-200 px-6 py-4 flex items-center gap-2">
             ...
             <span className="text-[10px] font-black uppercase tracking-widest text-slate-900">Recent Trade Enquiries</span>
          </div>
          <div className="flex-1 overflow-y-auto max-h-[400px]">
            <div className="divide-y divide-slate-100">
              {myEnq.map((e) => (
                <div key={e.id} className="p-5 hover:bg-slate-50 transition-colors group cursor-pointer">
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-[11px] font-black text-slate-900 bg-slate-100 px-2 py-0.5 rounded tracking-tighter">{e.ref}</span>
                    <Pill tone={e.status === "Won" ? "good" : "warn"}>{e.status}</Pill>
                  </div>
                  <div className="text-sm font-bold text-slate-900 group-hover:text-blue-600 transition-colors mb-1">{e.buyerCompany}</div>
      ...
      <div className="bg-slate-900 rounded-3xl p-8 md:p-12 text-white relative overflow-hidden shadow-2xl">
         ...
         <div className="relative z-10 max-w-2xl">
            <h3 className="text-2xl md:text-4xl font-black mb-4 tracking-tighter italic uppercase">Optimise Your Logistics</h3>
            <p className="text-slate-400 text-sm md:text-lg leading-relaxed opacity-90 font-medium">
               Your current inventory level in Park Royal Hub is at 64%. Vendors with {'>'}80% stock consistency are 3x more likely to win high-volume hospitality contracts.
            </p>
            <div className="mt-8 flex flex-wrap gap-4">
               <button className="mela-button-primary px-8 py-3 rounded-xl">
                  View Stock Report
               </button>
