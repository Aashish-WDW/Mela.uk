...
function BuyerOrders() {
  const list = orders.filter((o) => o.buyerCompany === "Hawksmoor Group" || o.buyerCompany === "Park Hospitality");
  return (
    <div>
      ...
      <div className="grid md:grid-cols-2 gap-4">
        {list.map((o) => (
          <div key={o.id} className="rounded-2xl border border-border bg-card p-5">
            ...
            <div className="text-sm text-muted-foreground">{o.items} line items · ETA {o.eta}</div>
            <div className="mt-4 h-2 rounded-full bg-muted overflow-hidden">
              <div className="h-full bg-foreground" style={{ width: o.status === "Confirmed" ? "20%" : o.status === "Picking" ? "50%" : o.status === "Dispatched" ? "80%" : "100%" }} />
            </div>
