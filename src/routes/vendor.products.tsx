...
import { Search, Plus, Filter, Edit3, Trash2, Package, CheckCircle, AlertTriangle, XCircle, Info } from "lucide-react";
...
function VendorProducts() {
  ...
  const list = mine.filter((p) => {
    ...
    const m = q === "" || p.name.toLowerCase().includes(q.toLowerCase()) || p.sku.toLowerCase().includes(q.toLowerCase());
    return ok && m;
  });
  ...
  return (
    <div className="space-y-8">
           ...
           <div className="flex gap-1 overflow-x-auto pb-1 scrollbar-hide">
             {tabs.map((t) => (
                ...
                >
                  ...
                  {t.label}
                  <span className={`ml-1 px-1.5 py-0.5 rounded-md ${tab === t.key ? 'bg-white/10 text-white' : 'bg-slate-100 text-slate-500'}`}>{counts[t.key]}</span>
                </button>
             ))}
           </div>
          ...
          <table className="w-full text-sm min-w-[1000px]">
            <thead>
              <tr className="text-left bg-gray-50/50 border-b border-gray-100">
                <th className="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-gray-400">Merchant SKU</th>
                <th className="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-gray-400">Commercial Name</th>
                <th className="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-gray-400">Unit Type</th>
                <th className="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-gray-400 text-center">MOQ</th>
                <th className="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-gray-400 text-center">Hub Stock</th>
                <th className="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-gray-400">Lead Time</th>
              ...
              {list.map((p) => {
                ...
                return (
                  <tr key={p.id} className="hover:bg-gray-50/80 transition-colors group">
                    <td className="px-6 py-5 font-black text-[11px] text-slate-900 uppercase tracking-widest">{p.sku}</td>
                    <td className="px-6 py-5">
                      <div className="flex items-center gap-4">
                        ...
                        <span className="font-bold text-slate-900 truncate group-hover:text-blue-600 transition-colors uppercase tracking-tight">{p.name}</span>
                      </div>
                    </td>
                    ...
                    <td className="px-6 py-5 text-center">
                       <span className={`font-black text-lg ${s === 'out' ? 'text-red-600' : s === 'low' ? 'text-amber-600' : 'text-emerald-700'}`}>
                          {p.stock.toLocaleString()}
                       </span>
                    </td>
                    <td className="px-6 py-5 text-gray-500 font-bold text-xs uppercase tracking-tighter">{p.leadTime}</td>
                    <td className="px-6 py-5">
              ...
              {list.length === 0 && (
                <tr>
                  <td colSpan={8} className="px-6 py-20 text-center">
                    ...
                    <div className="text-sm font-bold text-gray-400 uppercase tracking-widest">No results found in fleet</div>
                  </td>
                </tr>
              )}
            </tbody>
      ...
      <ProductForm
        ...
        product={editing}
        onSaved={(name) => { setOpen(false); flash(`${name} successfully Provisioned`); }}
      />
...
function ProductForm({
...
}: { open: boolean; onClose: () => void; product: Product | null; onSaved: (name: string) => void }) {
  ...
  return (
    <Modal
      ...
      description={product ? `Operational Update for ${product.sku}` : "Add a new B2B trade listing to the MELA platform."}
      size="lg"
    >
      <form
        onSubmit={(e) => { e.preventDefault(); onSaved(name || sku || "Product"); }}
        className="grid md:grid-cols-2 gap-6"
      >
        <div className="md:col-span-2">
          <Field label="Commercial Product Name">
            <input required className="mela-input" value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. Organic Heritage Flour" />
          </Field>
        </div>
        <Field label="Merchant SKU Code">
          <input required className="mela-input" value={sku} onChange={(e) => setSku(e.target.value)} placeholder="e.g. BP-FLOUR-01" />
        </Field>
        <Field label="Production Origin">
          <input className="mela-input" value={origin} onChange={(e) => setOrigin(e.target.value)} placeholder="e.g. United Kingdom" />
        </Field>
        <div className="md:col-span-2">
          <Field label="Trade Unit Configuration">
            <input required className="mela-input" value={unit} onChange={(e) => setUnit(e.target.value)} placeholder="e.g. Pallet of 40 × 25kg bags" />
          </Field>
        </div>
        <Field label="Minimum Order Quantity (MOQ)">
          <input required type="number" min={1} className="mela-input" value={moq} onChange={(e) => setMoq(Number(e.target.value))} />
        </Field>
        <Field label="Warehouse Stock Allocation">
          <input required type="number" min={0} className="mela-input" value={stock} onChange={(e) => setStock(Number(e.target.value))} />
        </Field>
        <div className="md:col-span-2">
          <Field label="Operational Lead Time">
            <input required className="mela-input" value={lead} onChange={(e) => setLead(e.target.value)} placeholder="e.g. 48h to London Hub" />
          </Field>
        </div>
        <div className="md:col-span-2">
          <Field label="Product Image URL">
            <input className="mela-input" value={image} onChange={(e) => setImage(e.target.value)} placeholder="https://images.unsplash.com/photo-..." />
          </Field>
        </div>
        <div className="md:col-span-2 flex justify-end gap-3 pt-6 border-t border-slate-100">
          <button type="button" onClick={onClose} className="px-6 py-2.5 rounded-xl border border-slate-200 text-sm font-bold text-slate-500 hover:bg-slate-50 transition-colors">Abort</button>
