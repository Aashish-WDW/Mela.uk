    ...
    <div className="space-y-8">
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-4 flex flex-col md:flex-row gap-4 items-center justify-between shadow-sm">
         <div className="relative w-full md:w-96">
            ...
            <input placeholder="Filter procurement partners..." className="w-full pl-10 pr-4 py-2 bg-gray-50 border border-gray-300 rounded-md text-sm focus:ring-1 focus:ring-amazon-orange outline-none transition-all" />
         </div>
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
        {buyers.map((b) => (
          <div key={b.id} className="bg-white border border-gray-200 rounded-lg shadow-sm hover:shadow-md transition-all group overflow-hidden flex flex-col">
            <div className="p-6 flex-1">
              <div className="flex items-start justify-between gap-4 mb-4">
                <div className="flex items-center gap-4">
                   <div className="w-14 h-14 rounded border border-gray-100 p-0.5 bg-white flex items-center justify-center overflow-hidden shrink-0 shadow-sm">
                      <img src={b.image} alt={b.name} className="w-full h-full object-cover rounded-sm" />
                   </div>
                   <div className="min-w-0">
                      <h3 className="font-bold text-lg text-amazon-dark truncate group-hover:text-blue-600 transition-colors">{b.name}</h3>
              ...
              <div className="space-y-3 mt-6">
                 ...
                 <div className="flex items-center gap-3 text-xs text-gray-600">
                    ...
                    <span className="font-medium truncate">{b.contact.toLowerCase().replace(' ', '.')}@business.uk</span>
                 </div>
              </div>
      ...
      <Modal open={open} onClose={() => setOpen(false)} title="Buyer Entity Onboarding" description="Provision a verified institutional trade account." size="lg">
        <form
          onSubmit={(e) => {
            e.preventDefault();
        ...
        >
          <Field label="Legal Entity Name"><input required className="amazon-input" value={name} onChange={(e) => setName(e.target.value)} placeholder="Full business name" /></Field>
          <Field label="Primary Operations Contact"><input required className="amazon-input" value={contact} onChange={(e) => setContact(e.target.value)} placeholder="Full name of lead buyer" /></Field>
          <Field label="Corporate Email Address"><input required type="email" className="amazon-input" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="procurement@company.uk" /></Field>
          <Field label="Industry Segment">
            <select className="amazon-input" value={segment} onChange={(e) => setSegment(e.target.value)}>
          ...
          <div className="md:col-span-2 flex justify-end gap-3 pt-4 border-t border-gray-50">
            ...
            <button type="submit" className="amazon-button-primary px-8">Confirm Entity Provisioning</button>
          </div>
        </form>
      </Modal>
