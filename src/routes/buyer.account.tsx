...
import { User, Shield, MapPin, Bell, Building2, CreditCard, Mail, Phone, Briefcase } from "lucide-react";
        ...
        <div className="lg:col-span-2 space-y-8">
          <Section title="Primary Representative" icon={<User className="w-5 h-5 text-blue-500" />}>
            <div className="grid md:grid-cols-2 gap-6 p-2">
              ...
              <Row k="Corporate Role" v="Procurement Manager" icon={<Briefcase className="w-4 h-4" />} />
              <Row k="Direct Email" v="sarah@hawksmoor.example" icon={<Mail className="w-4 h-4" />} />
              <Row k="Phone Link" v="+44 20 7946 0123" icon={<Phone className="w-4 h-4" />} />
            </div>
          </Section>

          <Section title="Logistics & Delivery Hubs" icon={<MapPin className="w-5 h-5 text-blue-500" />}>
            <div className="space-y-4 p-2">
              <AddressRow label="Corporate HQ" address="155 Commercial St, London E1 6BJ" type="Headquarters" />
              <AddressRow label="Spitalfields Hub" address="32 Brushfield St, London E1 6AT" type="Distribution" />
              <AddressRow label="Borough Market" address="49 Park St, London SE1 9EE" type="Retail Outlet" />
            </div>
          </Section>
        </div>

        {/* Right Column: Trade & Security */}
...
function Row({ k, v, icon }: { k: string; v: string; icon?: React.ReactNode }) {
  return (
    <div className="flex flex-col gap-1 py-2 group">
      <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest group-hover:text-blue-500 transition-colors">{k}</span>
...
function CompactRow({ k, v }: { k: string; v: string }) {
  return (
    <div className="flex items-center justify-between py-3 border-b border-slate-100 last:border-0 group">
      <span className="text-xs font-black text-slate-400 uppercase tracking-widest group-hover:text-blue-500 transition-colors">{k}</span>
  ...
  return (
    <div className="flex items-start justify-between p-4 rounded-2xl bg-slate-50 border border-slate-100 hover:border-blue-500/30 hover:bg-blue-50 transition-all group">
      <div className="flex gap-4">
        ...
        <div>
          <div className="text-sm font-black text-slate-900 group-hover:text-blue-600 transition-colors">{label}</div>
