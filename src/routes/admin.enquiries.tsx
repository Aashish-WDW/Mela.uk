...
import { MessageSquare, Search, Filter, User, MoreHorizontal, ArrowRight, Clock } from "lucide-react";
...
function AdminEnquiries() {
  ...
  const open = (e: typeof enquiries[number]) => {
    setEditing(e); setAssignee(e.assignee); setStatus(e.status); setNote("");
  };
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="bg-white border border-gray-200 rounded-lg shadow-sm overflow-hidden">
        <div className="p-4 md:p-6 border-b border-gray-100 flex flex-col md:flex-row gap-4 items-center justify-between">
           <div className="relative w-full md:w-96">
              ...
              <input placeholder="Search enquiries by reference or company..." className="w-full pl-10 pr-4 py-2.5 bg-gray-50 border border-gray-300 rounded-md text-sm focus:ring-1 focus:ring-amazon-orange outline-none transition-all shadow-inner" />
           </div>
          ...
          <table className="w-full text-sm min-w-[1000px]">
            <thead>
              <tr className="text-left bg-gray-50/50 border-b border-gray-100">
                <th className="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-gray-400">Trade Ref</th>
                <th className="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-gray-400">Buyer Entity</th>
                <th className="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-gray-400">Product SKU</th>
                <th className="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-gray-400 text-center">Volume</th>
                <th className="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-gray-400">Inbound Date</th>
                <th className="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-gray-400">Assignment</th>
                <th className="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-gray-400">Pipeline</th>
              ...
              {enquiries.map((e) => {
                ...
                return (
                  <tr key={e.id} className="hover:bg-gray-50/80 transition-colors group">
                    ...
                    <td className="px-6 py-5">
                       <div className="font-bold text-amazon-dark group-hover:text-blue-600 transition-colors">{e.buyerCompany}</div>
                       <div className="text-[10px] text-gray-400 font-medium uppercase tracking-tight">Verified Trade Buyer</div>
                    </td>
                    <td className="px-6 py-5">
                       <div className="font-medium text-gray-600 line-clamp-1 max-w-[200px]">{p?.name}</div>
                       <div className="text-[10px] text-gray-400 font-mono uppercase tracking-tighter">{p?.sku}</div>
                    </td>
                    ...
                    <td className="px-6 py-5">
                       <div className="flex items-center gap-2">
                          <div className="w-8 h-8 rounded-full overflow-hidden border border-gray-100 shrink-0 shadow-sm bg-gray-50 flex items-center justify-center">
                             {(() => {
                                const s = staff.find(st => e.assignee.startsWith(st.name.split(' ')[0]));
                                return s ? (
      ...
      <Modal open={!!editing} onClose={() => setEditing(null)} title="Operational Assignment" description={`Manage Procurement Desk for ${editing?.ref ?? ""}`} size="lg">
        <div className="mb-6 p-4 bg-gray-50 rounded-md border border-gray-100 flex items-center gap-4">
           ...
           <div>
              ...
              <div className="text-[10px] text-gray-400 font-bold uppercase tracking-widest">Inbound Request: {editing?.date}</div>
           </div>
        </div>
        ...
        >
          <Field label="Trade Executive Assignment">
            <select className="amazon-input" value={assignee} onChange={(e) => setAssignee(e.target.value)}>
          ...
          <Field label="Deal Pipeline Status">
            <select className="amazon-input" value={status} onChange={(e) => setStatus(e.target.value)}>
              <option>New</option>
          ...
          <div className="flex justify-end gap-3 pt-4 border-t border-gray-50">
            <button type="button" onClick={() => setEditing(null)} className="px-6 py-2.5 rounded-md border border-gray-300 text-sm font-bold text-gray-600 hover:bg-gray-50 transition-colors">Abort</button>
